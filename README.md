...
All layers run through the .NET orchestrator (from `src/`):

| Command | Layer | What it does |
|---------|-------|--------------|
| `dotnet run download` | 1 — data | Fetch S&P 500 prices (FMP) + SPY constituents → `data/raw/`, `data/constituents.json` (needs `FMP_API_KEY`) |
| `dotnet run simulate` | 2 — simulation | Backtest the $10M portfolio, label every lot-day → `data/lots.csv` |
| `dotnet run mlnet-all` | 3 — ML | CV all 5 models on both targets, test the champions → `data/artifacts-mlnet/` |
